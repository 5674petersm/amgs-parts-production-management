...
import { getEngineeringEmails } from "@/lib/permissions";
...
export async function notifyEngineeringMissingFinalStation(options: {
  ...
  const recipients = getEngineeringEmails();
  if (recipients.length === 0) {
    throw new Error(
      "No engineering recipients configured. Set AUTH_ENGINEER_EMAILS in the environment.",
    );
  }

  const { item, reportedBy } = options;
  const editLink = partEditUrl(item.itemId);
  const subject = `Final station needed – ${item.masterPNo} (Item ${item.itemId})`;
  const text = [
